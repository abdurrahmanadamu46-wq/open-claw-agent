 ...
 */
export function loadTemplate(industry, platform, purpose = 'full_brief') {
 ...
 */
export function render(template, variables, ragContext) {
    ...
    const replace = (s) => s.replace(VAR_REGEX, (_, name) => merged[name] ?? '');
    return {
 ...
 */
export function buildRagContext(contents, maxItems = 20) {
    return contents
        .slice(0, maxItems)
        .map((c) => (c.text ? `[${c.contentType}] ${c.text}` : ''))
        .filter(Boolean)
...
export function getSemanticBoundaryInstructions(industryTemplateId) {

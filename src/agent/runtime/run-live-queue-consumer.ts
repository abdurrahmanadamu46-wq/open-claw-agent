...
import { buildLiveQueueConsumerReport, loadLiveQueueConsumerPolicy } from './live-queue-consumer.js';
...
function main() {
  ...
  const policy = loadLiveQueueConsumerPolicy(policyPath);
  const report = buildLiveQueueConsumerReport(policy);

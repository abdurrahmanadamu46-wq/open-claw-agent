...
import { buildRuntimeExecutorBridgeReport } from './executor-bridge.js';
...
function main() {
  ...
  const bridgeReport = buildRuntimeExecutorBridgeReport(shadowReport);

  if (outPath) {
    writeFileSync(outPath, JSON.stringify(bridgeReport, null, 2));
  }

  console.log(JSON.stringify(bridgeReport, null, 2));
}

...
import { compileIndustryWorkflowBlueprint } from './industry-workflow.js';

const outPath = process.argv.includes('--out')
  ? path.resolve(process.argv[process.argv.indexOf('--out') + 1] ?? '')
  : path.resolve('F:/openclaw-agent/docs/architecture/LOBSTER_INDUSTRY_WORKFLOW_SMOKE_2026-03-30.json');

const blueprint = compileIndustryWorkflowBlueprint({
...
console.log(JSON.stringify({ ok: true, outPath, stepCount: blueprint.businessSteps.length }, null, 2));

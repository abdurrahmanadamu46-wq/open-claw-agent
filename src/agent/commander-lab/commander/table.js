...
const defaultDecisionTablePath = path.join(
  __dirname,
  "config",
  "decision-table.json"
);
 ...
 * @returns {string}
 */
function resolveDecisionTablePath(candidate) {
  if (candidate) {
...
function validateDecisionTable(table) {
  ...
  for (const rule of table.overrideRules) {
    ...
    if (!Number.isInteger(rule.priority) || rule.priority < 0) {
      throw new Error(`Override rule ${rule.id} must define a non-negative integer priority.`);
    }

    if (typeof rule.weight !== "number" || rule.weight < 0) {
      throw new Error(`Override rule ${rule.id} must define a non-negative numeric weight.`);
    }
 ...
 */
function readDecisionTable(filePath) {
...
/**
 * Load the Commander decision table from JSON.
 * The loader automatically refreshes if the file mtime changes.
 *
 * @param {{ tablePath?: string, forceReload?: boolean }} [options]
 * @returns {Record<string, any>}
 */
export function loadDecisionTable(options = {}) {
  ...
  return decisionTableCache.table;
}

/**
 * @returns {{ path: string, mtimeMs: number, version: string|null }}
 */
export function getDecisionTableInfo() {
  const table = loadDecisionTable();

  return {
    path: decisionTableCache.path,
...
export function getDecisionTableSchemaPath() {
  return defaultDecisionTableSchemaPath;
}

export function invalidateDecisionTableCache() {
  decisionTableCache = {
    path: null,
    mtimeMs: 0,
    table: null

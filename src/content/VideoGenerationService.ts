...
export const TEMPLATE_MAP: Record<string, PacingTemplate> = {
  '10秒爆款短视频': {
    pacing: 'fast',
    max_words: 45,
 ...
 */
export function buildSystemPromptForPacing(templateKey: string): string {
 ...
 */
export function parseFinalScriptFromLLM(raw: string): FinalScript {
  const trimmed = raw.trim();
  let data: unknown;
  const arrayMatch = trimmed.match(/\[[\s\S]*\]/);
  if (arrayMatch) {
    try {
      data = JSON.parse(arrayMatch[0]) as unknown;
    } catch {
      data = null;
    }
  }
  if (!Array.isArray(data)) {
    try {
      const obj = JSON.parse(trimmed) as { scenes?: unknown[]; script?: unknown[]; clips?: unknown[] };
  ...
  return list
    ...
    .map((item) => ({
      ...
      duration_estimate:
        typeof item.duration_estimate === 'number'
 ...
 */
export async function generateScriptByPacing(
  templateKey: string,
  input: {
    sellingPoints: string[];
    hook?: string;
 ...
 */
export function finalScriptToScriptOutput(finalScript: FinalScript): {
  scenes: Array<{ text: string; durationSeconds?: number; visual_prompt?: string }>;
